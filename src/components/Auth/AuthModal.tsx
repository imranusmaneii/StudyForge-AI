...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const userName = mode === 'signup' ? name.trim() : (email.split('@')[0] || 'Student');
      const newUser: User = {
  ...
  const handleGoogleSubmit = (e: React.FormEvent) => {
    ...
    if (!googleEmail || !googleEmail.includes('@')) {
      setError('Please enter your valid Google account email.');
      return;
    }
    ...
    setTimeout(() => {
      setIsLoading(false);
      const isNew = mode === 'signup' || mode === 'google';
      const rawHandle = googleEmail.split('@')[0] || 'User';
      const derivedName = googleName.trim() || (rawHandle.charAt(0).toUpperCase() + rawHandle.slice(1));
  ...
  const startGoogleSignIn = () => {
    setError('');
    // Pre-fill email/name if already typed in form
        ...
        {isGoogleFlow ? (
          ...
          <div className="space-y-5 animate-fade-in">
            <div className="text-center">
              ...
              <h3 className="text-xl font-bold text-white">Google Account Sign-In</h3>
              <p className="text-xs text-gray-400 mt-1">
                Enter your Google account email to sign in or create a fresh workspace.
              </p>
            </div>
            ...
            <form onSubmit={handleGoogleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-300 mb-1.5">Your Google Gmail Address *</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-300 mb-1.5">Full Name (Optional)</label>
                <div className="relative">
              ...
              <div className="pt-2 flex items-center gap-3">
                <button
                  ...
                  className="w-1/3 py-2.5 px-3 rounded-xl bg-white/5 hover:bg-white/10 text-xs font-semibold text-gray-300 transition-all"
                >
                  Back
                </button>
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-2/3 py-2.5 px-4 rounded-xl bg-[#0070F3] hover:bg-[#0070F3]/90 text-white font-semibold text-xs sm:text-sm shadow-[0_0_20px_rgba(0,112,243,0.3)] transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {isLoading ? (

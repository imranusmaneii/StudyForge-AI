...
import { X, Mail, Lock, User as UserIcon, Sparkles, LogIn, UserPlus, AlertCircle, ExternalLink } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleGoogleSignIn = async () => {
    setError('');
    setIsLoading(true);
    console.log('[AuthModal] User clicked "Sign in with Google" button');

    try {
      const user = await signInWithGoogle();
      console.log('[AuthModal] Google authentication succeeded for user:', user.email);
      onLogin(user, mode === 'signup');
      onClose();
    } catch (err: any) {
      console.error('[AuthModal] Google Auth Error captured:', err);
      setError(err.message || 'Google sign-in failed. Try email sign-in or open app in a new tab.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
      <GlareHover
        ...
        className="w-full max-w-sm max-h-[90vh] overflow-y-auto no-scrollbar shadow-[0_0_50px_rgba(0,112,243,0.25)] text-white"
      >
        <div className="relative p-5 sm:p-6 w-full">
          ...
          <button
            onClick={onClose}
            className="absolute top-3.5 right-3.5 p-1.5 rounded-xl text-gray-400 hover:text-white hover:bg-white/5 transition-all z-20"
          >
            <X className="w-4 h-4" />
          </button>
          ...
          {/* Error Alert */}
          {error && (
            <div className="mb-3 p-3 rounded-xl bg-red-500/10 border border-red-500/30 text-[11px] text-red-300 flex flex-col gap-2">
              <div className="flex items-center gap-2">
                <AlertCircle className="w-4 h-4 text-red-400 shrink-0" />
                <span>{error}</span>
              </div>
              {(error.toLowerCase().includes('popup') || error.toLowerCase().includes('google') || error.toLowerCase().includes('iframe')) && (
                <button
                  type="button"
                  onClick={() => window.open(window.location.href, '_blank')}
                  className="mt-1 flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-500/20 hover:bg-red-500/30 text-white font-medium text-[11px] transition-colors self-start border border-red-500/30"
                >

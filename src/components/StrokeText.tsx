...
export const StrokeText: React.FC<StrokeTextProps> = ({
  ...
  const fontStyle = useMemo(
    () => ({
      ...
      letterSpacing: typeof letterSpacing === 'number' ? `${letterSpacing}px` : String(letterSpacing)
    }),
  ...
  const effectiveAlign = align === 'responsive' ? (isDesktop ? 'left' : 'center') : align;
  const aspectAlign = effectiveAlign === 'center' ? 'xMidYMid' : effectiveAlign === 'right' ? 'xMaxYMid' : 'xMinYMid';
  ...
  return (
    ...
    >
      <svg className="stroke-text__svg" viewBox={viewBox} preserveAspectRatio={`${aspectAlign} meet`} aria-hidden="true">
        {fillMode === 'wipe' && box && (
          <defs>
